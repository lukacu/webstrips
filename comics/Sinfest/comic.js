...
var IMAGE_RE = new RegExp("<img src\\=\"http\\://sinfest\\.net/comikaze/comics/(\\d{4}-\\d{2}-\\d{2})\\.gif\"");

var TITLE_RE = new RegExp("comikaze/comics/\\d{4}-\\d{2}-\\d{2}\\.gif\" alt=\"([^\"]*)\"");

var ANCHOR_RE = new RegExp("href\\=\"http://sinfest\\.net/archive_page\\.php\\?comicID\\=(\\d+)\"><img src\\=\"images\\/prev_a\\.gif\"");

var ARCHIVE_RE = new RegExp("option value=\"(\\d{1,4})\"> \\[\\d+\\] ([^<]+)<", "g");
...
function title(id) {
	...
	s = get("http://sinfest.net/archive_page.php?comicID=" + i);
...
function image(id) {
	...
	if (m) 
		return "http://sinfest.net/comikaze/comics/" + m[1] + ".gif";	
...
function newest() {
	...
	s = get("http://www.sinfest.net/index.php");
...
function link(id) {
	return "http://sinfest.net/archive_page.php?comicID=" + id;	
}
...
function archive(id) {
	s = get("http://www.sinfest.net/archive.php");
