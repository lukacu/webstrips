...
var PATTERN_RE = new RegExp("<img align=\"middle\" src\\=\"strips\\/(\\d{4})\\.(gif|jpg)\" alt=\"[^\"]*\" title=\"([^\"]*)\"");

var ARCHIVE_RE = new RegExp("\">([0-9]+) \\- ([^<]+)</a>", "g");
...
function title(id) {
	...
	s = get("http://www.megatokyo.com/strip/" + i);
...
function image(id) {
	...
	if (m) {
		return "http://www.megatokyo.com/strips/" + m[1] + "." + m[2];
	} else
		return null;
...
function link(id) {
	return "http://www.megatokyo.com/strip/" + id;
}
...
function archive(id) {
	...
	s = get("http://www.megatokyo.com/archive.php?list_by=date");

	i = parseInt(id);

	start = s.indexOf(format("strip/%04d\">%04d", i, i));

	ARCHIVE_RE.lastIndex = (start == -1) ? 0 : start;

	while (m = ARCHIVE_RE.exec(s)) {

		j = parseInt(m[1]);

		archiveAppend(j.toString(), m[2]);
